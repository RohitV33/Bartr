...
export function SkillOfferingCard({ className = '' }) {
  return (
    <motion.div
      ...
      className={`bg-bartr-surface rounded-2xl shadow-md border border-bartr-border p-4 w-full ${className}`}
    >
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs font-semibold text-bartr-muted uppercase tracking-wider font-sora">
          Skill Offering
        </span>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 bg-bartr-bg rounded-lg px-3 py-2 border border-bartr-border">
          ...
          <span className="text-xs text-bartr-muted font-semibold tracking-widest uppercase">Your Skill</span>
        </div>
        <div className="bg-bartr-bg rounded-lg px-3 py-2 border border-bartr-border flex items-start gap-2">
          ...
          <span className="text-xs text-bartr-muted">Short description…</span>
        </div>
      </div>
    ...
    >
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 bg-bartr-bg rounded-lg px-3 py-2 border border-bartr-border">
          ...
          <span className="text-xs text-bartr-muted font-semibold tracking-widest uppercase">Required Skill</span>
        </div>
        <select className="w-full text-xs text-bartr-muted bg-bartr-bg rounded-lg px-3 py-2 border border-bartr-border outline-none appearance-none">
...
export function SkillStatsCard({ className = '' }) {
  ...
  return (
    <motion.div
      ...
      className={`bg-bartr-surface rounded-2xl shadow-md border border-bartr-border p-4 w-full ${className}`}
    >
      <span className="text-xs font-semibold text-bartr-muted uppercase tracking-wider font-sora block mb-3">
        Skill Stats
      </span>
      <div className="flex gap-4 mb-4">
        <div className="flex-1 text-center">
          <p className="text-2xl font-bold text-bartr-text font-sora">247</p>
          <p className="text-[10px] text-bartr-muted mt-0.5">Exchanges Done</p>
        </div>
        <div className="w-px bg-bartr-border" />
        <div className="flex-1 text-center">
          <p className="text-2xl font-bold text-bartr-text font-sora">58</p>
          <p className="text-[10px] text-bartr-muted mt-0.5">Active Requests</p>
        </div>
      </div>
      <div className="space-y-2">
        {bars.map(b => (
          <div key={b.label} className="flex items-center gap-2">
            <span className="text-[10px] text-bartr-muted w-12 shrink-0">{b.label}</span>
            <div className="flex-1 h-1.5 bg-bartr-bg rounded-full overflow-hidden">
              <div className={`h-full ${b.color} rounded-full`} style={{ width: `${b.pct}%` }} />
            </div>
            <span className="text-[10px] text-bartr-muted w-7 text-right">{b.pct}%</span>
          </div>
        ))}
      </div>
    </motion.div>
