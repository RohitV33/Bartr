...
export default function ContactPage() {
  ...
  const handleSubmit = async (ev) => {
    ...
    window.open(
      `mailto:hello@bartr.io?subject=${encodeURIComponent(form.subject || 'Contact from Bartr')}&body=${encodeURIComponent(`Name: ${form.name}\nEmail: ${form.email}\n\n${form.message}`)}`,
      '_blank'
    )
    setLoading(false)
    setSent(true)
  ...
  const inputClass = (k) =>
    `w-full bg-bartr-bg border ${errors[k] ? 'border-red-400' : 'border-bartr-border'} rounded-xl px-4 py-3 text-sm text-bartr-text placeholder-bartr-muted outline-none focus:border-yellow-400 transition-colors`
  ...
  return (
    <PageCard>
      <div className="max-w-5xl mx-auto">
        ...
        >
          <span className="text-xs font-semibold tracking-widest uppercase text-indigo-500 font-sora">Get in touch</span>
          <h1 className="text-4xl md:text-5xl font-extrabold font-sora text-bartr-text mt-3 leading-tight">
            We'd love to hear<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-amber-500">from you.</span>
          </h1>
          <p className="text-bartr-muted mt-4 max-w-md mx-auto">
        ...
        <div className="grid md:grid-cols-5 gap-8 items-start">
          ...
          >
            {INFO.map(({ icon: Icon, label, value, sub }, i) => (
              <div key={i} className="bg-bartr-surface border border-bartr-border rounded-2xl p-5 flex items-start gap-4">
                <div className="w-10 h-10 bg-yellow-300/10 rounded-xl flex items-center justify-center shrink-0">
            ...
            {/* Social */}
            <div className="bg-gradient-to-br from-indigo-500/10 via-purple-500/10 to-transparent border border-bartr-border rounded-2xl p-5">
              <p className="text-xs font-semibold text-bartr-muted uppercase tracking-wider mb-3">Follow us</p>
              <div className="flex gap-3">
                {['Twitter', 'Instagram', 'LinkedIn'].map(s => (
                  <a key={s} href="#" className="text-xs font-semibold text-bartr-muted hover:text-bartr-text transition-colors bg-bartr-bg px-3 py-1.5 rounded-full border border-bartr-border">
                    {s}
                  </a>
                ))}
              </div>
          ...
          <motion.div
            ...
            className="md:col-span-3 bg-bartr-surface border border-bartr-border rounded-3xl p-8 shadow-xl"
          >
              ...
              {sent ? (
                ...
                >
                  ...
                  <button
                    ...
                    className="bg-yellow-300 text-bartr-dark text-sm font-bold px-6 py-2.5 rounded-full hover:bg-yellow-400 transition-all"
                ...
                >
                  <div className="mb-2">
                    ...
                    <p className="text-sm text-bartr-muted mt-0.5">Fill out the form and we'll be in touch.</p>
                  </div>
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full flex items-center justify-center gap-2 bg-bartr-dark text-white font-bold text-sm py-3.5 rounded-xl hover:bg-gray-800 active:scale-95 transition-all disabled:opacity-60 shadow-lg"
                    style={{ fontFamily: "'Sora', sans-serif" }}
                  >
                    {loading ? (
