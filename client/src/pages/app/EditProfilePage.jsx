...
function ProfileHero({ user, avatarRef, onAvatarClick, uploading, scrollY }) {
  ...
  return (
    <div ref={heroRef}
      ...
      className="relative overflow-hidden rounded-3xl border-2 border-bartr-border bg-bartr-surface shadow-[4px_4px_0px_var(--border)] dotted-bg mb-8"
    >
      <div className="relative flex flex-col items-center text-center px-8 pt-10 pb-10 z-10">
        {/* Avatar */}
        <div className="relative mb-5">
          <div className="ring-4 ring-bartr-border rounded-full shadow-2xl relative bg-bartr-surface overflow-hidden">
            <Avatar src={user?.avatar_url} name={user?.full_name} size="xl" />
          </div>
          <button type="button" onClick={onAvatarClick}
            className="absolute bottom-0 right-0 w-10 h-10 bg-bartr-text text-bartr-bg rounded-xl flex items-center justify-center border-2 border-bartr-border hover:bg-bartr-text/90 active:scale-90 transition-all shadow-md"
          >
            <Camera className="w-4 h-4 text-bartr-bg" />
          </button>
        ...
        <p className="text-bartr-muted text-sm mb-4 font-bold" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        {uploading && (
          <div className="mt-3 flex items-center gap-2 text-xs text-bartr-text bg-bartr-text/10 px-3 py-1.5 rounded-lg border-2 border-bartr-border font-bold">
            <div className="w-3 h-3 border-2 border-bartr-text border-t-transparent rounded-full animate-spin" />
            Uploading photo…
          </div>
        )}
      </div>
...
function FieldRow({ icon: Icon, children }) {
  return (
    <div className="flex items-start gap-3">
      <div className="w-9 h-9 mt-1 rounded-xl border-2 border-bartr-border bg-bartr-text/10 text-bartr-text flex items-center justify-center flex-shrink-0">
...
export default function EditProfilePage() {
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6">
      ...
      {/* Form */}
      <Reveal delay={80}>
        <div className="bg-bartr-surface rounded-2xl border-2 border-bartr-border shadow-[4px_4px_0px_var(--border)] p-6 space-y-6">

          {errors.root && (
            <div className="bg-red-500/10 border-2 border-red-500 rounded-xl px-4 py-3 flex items-start gap-2">
          ...
          <form onSubmit={handleSubmit(d => profileMutation.mutate(d))} className="space-y-5">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                ...
                className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all border-2 border-bartr-border flex items-center justify-center gap-2 shadow-[3px_3px_0px_var(--border)] active:translate-y-[2px] active:shadow-none
                  ${saved
                    ? 'bg-bartr-text text-bartr-bg'
                    : 'bg-bartr-text text-bartr-bg hover:bg-bartr-text/90 disabled:opacity-60'
                  }`}
                style={{ fontFamily: "'Sora', sans-serif" }}
              >
                {saved ? (
                  ...
                  <><div className="w-4 h-4 border-2 border-bartr-bg border-t-transparent rounded-full animate-spin" /> Saving…</>
                ) : (
                  'Save Changes'
                )}
              </button>
      ...
      <Reveal delay={120}>
        <p className="text-center text-xs text-bartr-muted font-bold mt-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
